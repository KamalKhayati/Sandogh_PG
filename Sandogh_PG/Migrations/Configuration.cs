namespace Sandogh_PG.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<Sandogh_PG.MyContext>
    {
        ...
        protected override void Seed(Sandogh_PG.MyContext context)
        {
            ...
            context.Entry(new TarifSandogh() { Id = 1, NameSandogh = "صندوق قرض الحسنه", IsDefault = true, AppActived=false, TarikhEjad= Convert.ToDateTime(DateTime.Now.ToString().Substring(0, 10)) }).State = context.TarifSandoghs.Any(s => s.Id == 1) ? EntityState.Unchanged : EntityState.Added;
